...
//基于动态路由的通配符*解析
//如果将所有的静态文件放在/usr/web目录下，那么filepath的值即是该目录下文件的相对地址。
//映射到真实的文件后，将文件返回，静态服务器就实现了
//找到文件后，如何返回这一步，net/http库已经实现了
//框架要做的，仅仅是解析请求的地址，映射到服务器上文件的真实地址，交给http.FileServer处理就好了。
...
//访问静态资源
//Static这个方法是暴露给用户的。用户可以将磁盘上的某个资源文件夹root，映射到路由relativePath
